...
//    lights.push_back(light2);
    ...
    for (int i = 0; i < 50; i++) {
        float x = glm::sin(i/(glm::pi<float>()/85)) * 10;
        float y = glm::cos(i/(glm::pi<float>()/85)) * 10;
        FAPointLight *l = new FAPointLight();
        l->setShader(shaders->getShader("PointLight"));
        l->setColor(FAColorBlack);
...
//    glClearColor(1, 1, 1, 0);
    ...
    for (FAPointLight *light : lights)
        light->onRender(deferredTextures, camera);
...
//        node->update(dt);
    ...
    for (FAPointLight *light : lights) {
        light->setRX(-camera->getRX());
        light->setRY(-camera->getRY());
        light->setRZ(-camera->getRZ());
        light->update(dt);
    }
